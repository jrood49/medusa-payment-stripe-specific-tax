...
abstract class StripeBase extends AbstractPaymentProcessor {
  ...
  async updatePayment(
    ...
    const lineItems = items.map((item) => {
      ...
      // const allocations = calculationContext.allocation_map[item.id] || {}
    ...
    // const shippingCost = calculationContext.shipping_methods.reduce(
    ...
    if (stripeId !== paymentSessionData.customer) {
      ...
      if (
        amountTotal &&
        paymentSessionData.amount === Math.round(amountTotal)
      ) {
        return;
      }
      try {
